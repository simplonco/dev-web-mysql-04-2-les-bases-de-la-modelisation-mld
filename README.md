...
![entités + relations + cardinalités = MCD complet](images/mcd-complet.png)
...
* Les _identifiants_ deviennent des **clés primaires**.

![entités = tables + champs](images/mcd-entites-tables-champs.png)
...
1. Comme tu l’as vu plus haut, un élève ne peut être inscrit que dans **0 à 1** école à la fois..
...
![la clé primaire de l'entité côté N devient clé étrangère dans l'entité côté 1](images/mld-cles-etrangeres.png)
...
* Soit tu crées une clé étrangère `pet_id` dans la table `wizard`.
* Soit tu crées une clé étrangère `wizard_id` dans la table `pet`.
...
![MLD complet](images/mld-complet.png)
